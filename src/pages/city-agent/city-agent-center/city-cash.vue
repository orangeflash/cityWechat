  ...
  <div class="withdraw-box">
    ...
    <header class="withdraw-index-header">
      <div class="left">
        ...
        <div class="sun-box"><span class="symbol">￥</span><input type="Number" @input="getServiceCharge" placeholder="0.00" class="input-sum"/>
        </div>
      </div>
      <div class="right" @click="goPointPage({name:'PartnerCashlist'})">提现明细</div>
    </header>
    <article v-for="(item,index) in forwardMethod" :key="index">
    ...
    <div class="private-info">
      <van-cell-group>
        <van-field v-if="+radio===1" v-model="wxNumber" input-align="right" label="微信账号"
                   placeholder="请输入微信账号" clearable/>

        <van-field v-if="+radio===3" v-model="bankCardId" type="number" input-align="right"
                   label="银行卡号" placeholder="请输入银行卡号" clearable/>
        <van-field v-if="+radio===3" v-model="bankCardInfo" input-align="right" label="支行信息"
                   placeholder="请输入支行信息" clearable/>

        <van-field v-if="+radio===2" v-model="payNumber" type="number" input-align="right"
    ...
    <footer>
      ...
      <p class="cost-legend">
        <em>详细说明 </em>
        <em>提现门槛为{{config.limit}}元</em>
        <em> 提现手续费为{{config.recharge}}%</em>
...
<script>
  ...
  import {showDialog, hideLoading, showLoading, showText, showHandleStatusByFlag, isTellPhone} from "@/util";
  import Header from "@/components/header";
  export default {
    name: 'partnerCash',
    components: {CheckBox,Header,AgreeAgreement,},
    data() {
      return {
        radio: 0,
        itemSum: 0,
    ...
    methods: {
      ...
      ...mapActions("storeAuxiliary/storeBalance", ["getStoreBalance","postStoreCash"]),
      async gethhrpz(){
        // this.config=await this.getPartnerconfig()
        this.config=(await this.getStoreConfig()).data
        console.log(this.config,)
      },
      async gethhrsj(){
        this.hhrsj=(await this.getStoreBalance({storeId:this.storeId})).money;
      ...
      getServiceCharge() {
        ...
        this.serviceCharge = (this.itemSum / 100 * this.config.recharge).toFixed(2);
      ...
      async getMoney() {
        ...
        if (+this.config.limit > +config.money ) return showText('未达到提现门槛');
        ...
        if (!isTellPhone(this.phoneTel)) return showText('请输入正确的联系方式');
        ...
        // return
        showLoading();
        const res = await this.postStoreCash(config);
        // hideLoading();
        if (res.code) {
    ...
    computed: {
      ...
      forwardMethod() {
        const arr = [];
        if (this.config.forwardMethod) {
          for (let p in this.config.forwardMethod) {
