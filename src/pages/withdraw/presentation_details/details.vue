<template>
  ...
  <div class="details">
    ...
    <van-cell-group>
      ...
      <van-cell title="账户余额："  :value="myInfo.balance+'元'" ></van-cell>
    </van-cell-group>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      ...mapActions("presentationDetails", ["getPresentationInfoById"]),
      async getPresentationById(){
        this.myInfo = await this.getUserInfoById(this.user.id);
        const data = await this.getPresentationInfoById({
