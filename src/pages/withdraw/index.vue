<template>
  <div class="withdraw-box">
    ...
    <header class="withdraw-index-header">
      <div class="left">
        <em class="usable-money">可提现金额￥{{myInfo.balance}}</em>
      ...
      <div class="right" @click="goPointPage({path:'/withdraw/presentationDetails'})">提现明细</div>
    </header>
    <article class="select-withdraw" v-for="(item,index) in balanceGetType" :key="index">
    ...
    <footer>
      ...
      <p class="cost-legend">
        <em>详细说明 </em>
        <em>提现门槛为{{config.minGetRecharge}}元</em>
        <em> 提现手续费为{{config.serviceRecharge}}%</em>
...
<script>
  ...
  import {showDialog, hideLoading, showLoading, showHandleStatusByFlag} from "../../util";
  ...
  export default {
    ...
    methods: {
      ...
      ...mapActions("withdraw", ["getReflectConfig", 'reflectMoney']),
      selectChannel() {
        return [{
      ...
      getServiceCharge() {
        ...
        this.serviceCharge = (this.itemSum / 100 * this.config.serviceRecharge).toFixed(2);
      ...
      async getMoney() {
        ...
        this.itemSum = document.querySelector('.input-sum').value;
        if (Number(this.itemSum) <= 0) return Toast('请输入提现金额');
        const config = {
          money: Number(this.itemSum).toFixed(2),
          mode: typeId,
        }

        if (+config.money > +this.myInfo.balance) return Toast('已超出可提现金额范围');
        switch (typeId) {
          case 1:
            if (!this.wxNumber.trim()) return Toast('请输入微信账号');
            config.accountNumber = this.wxNumber;
            break;
          case 2:
            if (!this.payNumber.trim()) return Toast('请输入支付宝账号');
            config.accountNumber = this.payNumber;
            break;
          case 3:
            if (!this.bankCardId.toString().trim()) return Toast('请输入银行卡号')
            if (!this.bankCardInfo.toString().trim()) return Toast('请输入支行信息');
            config.accountNumber = this.bankCardId;
            config.branch = this.bankCardInfo;
            break;
        }
        if (!this.userName.toString().trim()) return Toast('请输入收款人姓名');
        if (!this.phoneTel.toString().trim()) return Toast('请输入真实的联系方式');
        if (!this.checked) return Toast('请选择我已阅读并同意');
        config.name = this.userName;
        config.linkTel = this.phoneTel;
        showLoading();
        const res = await this.reflectMoney(config);
        hideLoading();
        if (res.code) {
    ...
    async created() {
      this.myInfo = await this.getUserInfoById(this.user.id);
      this.config = await this.getReflectConfig();
//      this.config[0]
    }
  }
</script>
