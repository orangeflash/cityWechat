    ...
    <scroller
      ...
      <div class="data-list">
        <div v-for="(item,key) in dataList" :key="key" class="persent"
             @click="goPointPage({name: 'PartnerCashdetail',params:{id:item.id}})">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions("partner/partnerEnter", ["getpartnerStatistics", "getpartnerCashlist"]),
      // 刷新的函数
      async onRefresh(done) {
        console.log('222222222')
        Object.assign(this, defaultPage());
        this.dataList = [];
        this.dataList = await this.getpartnerCashlist({
          page: this.pageNum,
          size: this.pageSize,
          type: this.type,
        });
      ...
      async onInfinite(done) {
        ...
        this.pageNum++;
        const list = await this.getpartnerCashlist({
          page: this.pageNum,
          size: this.pageSize,
          type: this.type,
        });
