  ...
  <dd-layout-div height="100vh" class="bg-f flex flex-col">
    ...
    <div class="flex-g-1 posi-r w100 posi-r">
      ...
      <scroller
        ...
        v-if="list.length > 0"
      >
        <city-love-cell :list="list" type="follow" @collection="(key)=>list.splice(key,1)"/>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions("cityLove", ['cityLoveGetFollowInfo']),
      changeTab(index = 0) {
      ...
      async getDataList(refresh) {
        ...
        let result = await this.cityLoveGetFollowInfo(this.params);
